...
public class SyntaxAuditTest {
    ...
    // ---------- 集合 ----------
    @Test void listIndex() throws Exception { assertEquals(2, num("var.l = [1,2,3]\nreturn l[1]")); }
    @Test void listLen() throws Exception { assertEquals(3, num("var.l = [1,2,3]\nreturn l.length")); }
    @Test void mapGet() throws Exception { assertEquals(7, num("var.m = {'a': 7}\nreturn m['a']")); }
    @Test void nestedColl() throws Exception { assertEquals(9, num("var.x = [{'k':[0,9]}]\nreturn x[0]['k'][1]")); }
    @Test void listAppendEmptyIndex() throws Exception { assertEquals(5, num("var.l = [1]\nl[] = 5\nreturn l[1]")); }

    // ---------- 命名空间 ----------
    @Test void varNs() throws Exception { assertEquals(1, num("var.x = 1\nreturn var.x")); }
    @Test void valNs() throws Exception { assertEquals(2, num("val.x = 2\nreturn val.x")); }
    @Test void valBareRead() throws Exception { assertEquals(5, num("val.x = 5\nreturn x")); }
    @Test void valImmutableReassignThrows() throws Exception {
        // val 不可变:脚本重赋报错
        assertThrows(Exception.class, () -> num("val.x = 1\nval.x = 2\nreturn val.x"));
    }
    @Test void valHostForceSetThenImmutable() throws Exception {
        ...
        ctx2.forceSetLocalValue(priv.seventeen.artist.aria.context.VariableKey.of("HP"),
                new priv.seventeen.artist.aria.value.NumberValue(100));
        assertThrows(Exception.class, () -> Aria.eval("val.HP = 1\nreturn val.HP", ctx2));
    }
    ...
    // ---------- 自增自减 ----------
    @Test void prefixInc() throws Exception { assertEquals(2, num("var.i = 1\nreturn ++i")); }
    @Test void postfixInc() throws Exception { assertEquals(1, num("var.i = 1\nvar.j = i++\nreturn j")); }
    @Test void prefixDec() throws Exception { assertEquals(0, num("var.i = 1\nreturn --i")); }
    ...
    @Test void compoundAssign() throws Exception {
        assertEquals(15, num("var.x = 10\nx += 5\nreturn x"));
        assertEquals(20, num("var.x = 10\nx *= 2\nreturn x"));
        assertEquals(2, num("var.x = 10\nx /= 5\nreturn x"));
        assertEquals(0xF0, num("var.x = 0xFF\nx &= 0xF0\nreturn x"));
    }
    ...
    @Test void closure() throws Exception { assertEquals(15, num("var.x = 10\nvar.f = -> { return args[0] + x }\nreturn f(5)")); }
    ...
    @Test void javaUseStatic() throws Exception {
        assertEquals(9.0, num("val.Math = use('java.lang.Math')\nreturn Math.max(4, 9)"), 1e-9);
    }
    @Test void javaInstance() throws Exception {
        assertEquals(2, num("val.AL = use('java.util.ArrayList')\nval.l = AL()\nl.add(1)\nl.add(2)\nreturn l.size()"));
    }
    ...
    @Test void spreadList() throws Exception { assertEquals(4, num("var.a = [1,2]\nvar.b = [...a, 3, 4]\nreturn b.length")); }
    ...
    @Test void instanceofInFunctionBody() throws Exception { // 验证 executeInlineInternal 也接通
        assertTrue(bool("class A { }\nvar.f = -> { return args[0] instanceof A }\nval.a = A()\nreturn f(a)"));
    }
