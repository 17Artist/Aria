...
public class JavaNativeBenchmark {
    ...
    /** 互递归 fib 基线：两个相互调用的 static double 方法，对等 Aria 互递归整组 JIT 编译产物。 */
    @Benchmark
    public double mutualFibonacci25() {
        return fa(25);
    }

    private static double fa(double n) {
        if (n <= 1) return n;
        return fb(n - 1) + fb(n - 2);
    }

    private static double fb(double n) {
        if (n <= 1) return n;
