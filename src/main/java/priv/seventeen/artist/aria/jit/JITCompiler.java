...
public class JITCompiler {
        ...
        for (IRInstruction inst : code) {
            switch (inst.opcode) {
                ...
                case CALL:
                    ...
                    break;
                case CALL_STATIC:
                    // math.*/io.* 内联、ns.method（rtCallByNameCached）、裸名（自递归 / 兄弟函数 /
                    // 任意 var 函数）都允许：doCompile 对纯数值组走整组编译，否则通用路径以 rtCallByName
                    // 动态分派（正确）。仅 name 为 null 无法解析时拒绝。
    ...
    private boolean isFastDoubleEmittable(IRInstruction[] code) {
        for (IRInstruction inst : code) {
            switch (inst.opcode) {
                case LOAD_CONST, LOAD_NONE, LOAD_TRUE, LOAD_FALSE, LOAD_VAR, LOAD_ARG:
                case ADD, SUB, MUL, DIV, MOD, NEG, INC, DEC:
                case ADD_NUM, SUB_NUM, MUL_NUM, DIV_NUM, MOD_NUM:
    ...
    private Map<Integer, Integer> buildGroupCallTargets(IRProgram m, Map<String, Integer> nameToIndex) {
        ...
        for (int pc = 0; pc < code.length; pc++) {
            ...
            if (calleeName.indexOf('.') >= 0) {
                // 命名空间调用：仅允许 emitFastDoubleBytecode 能内联的 math.*；其余（io.print 等）令组不可编译。
                if (calleeName.startsWith("math.") && isSupportedStaticCall(inst)) continue;
                return null;
            }
    ...
    /**
     * 裸标识符读取的运行期解析:scope→var→val(已赋值)→在 scope 创建。与解释器 LOAD_SCOPE 处理一致。
     * JIT 生成代码调用此方法,使编译后的闭包能正确读到外层 var/val(此前仅查 scope → 返回 none)。
     */
    public static IValue<?> rtLoadScope(Context ctx, VariableKey key) {
        VariableReference sr = ctx.getScopeStack().getExisting(key);
        if (sr != null) return sr.getValue();
        VariableReference vr = ctx.getLocalStorage().getVarVariableExisting(key);
        if (vr != null) return vr.getValue();
        ValueReference valr = ctx.getLocalStorage().getValVariableExisting(key);
        if (valr != null && valr.isAssigned()) return valr.getValue();
        return ctx.getScopeStack().get(key).getValue();
    }

    /**
     * 裸标识符写入的运行期解析:写已存在的 scope 绑定;否则写已存在的 var 绑定(对应解释器 LOAD_SCOPE
     * 把 var 解析结果缓存进 scopeRefs、后续 STORE_SCOPE 复用该 var ref 的"读改写更新 var"效果);否则在
     * scope 创建。修复:JIT 闭包对捕获 var 做 `x += a` 时,此前 STORE_SCOPE 只写 scope → var 不更新。
     */
    public static void rtStoreScope(Context ctx, VariableKey key, IValue<?> value) {
        VariableReference sr = ctx.getScopeStack().getExisting(key);
        if (sr != null) { sr.setValue(value); return; }
        VariableReference vr = ctx.getLocalStorage().getVarVariableExisting(key);
        if (vr != null) { vr.setValue(value); return; }
