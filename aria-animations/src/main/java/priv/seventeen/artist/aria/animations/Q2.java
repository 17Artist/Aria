...
public class Q2 implements IAriaObject {
    ...
    @AriaObjectConstructor("Q2")
    public Q2(InvocationData data) {
        if (data.size() >= 4) {
    ...
    public double get(InvocationData data) {
        ...
        double value = switch (type) {
            case 1 -> quadOut(progress);
            case 2 -> quadInOut(progress);
            default -> quadIn(progress);
        };
    ...
    private double quadIn(double t) {
        return t * t;
    }


    private double quadOut(double t) {
        return -(t * (t - 2));
    }


    private double quadInOut(double t) {
        if (t < 0.5) {
            return 2 * t * t;
        } else {
            t = 2 * t - 1;
