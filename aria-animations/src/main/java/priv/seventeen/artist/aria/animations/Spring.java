...
public class Spring implements IAriaObject {
    ...
    @AriaObjectConstructor("Spring")
    public Spring(InvocationData data) {
        if (data.size() >= 5) {
            this.start = data.get(0).doubleValue();
            this.end = data.get(1).doubleValue();
            this.tension = data.get(2).doubleValue();
            this.damping = data.get(3).doubleValue();
    ...
    public double get(InvocationData data) {
        ...
        double decay = Math.exp(-damping * progress);
        double oscillation = Math.cos(Math.sqrt(tension) * progress);
        double value = end - (decay * oscillation * distance);
