...
public class Sine implements IAriaObject {
    ...
    @AriaObjectConstructor("Sine")
    public Sine(InvocationData data) {
        if (data.size() >= 4) {
    ...
    public double get(InvocationData data) {
        ...
        double value = switch (type) {
            case 1 -> sineOut(progress);
            case 2 -> sineInOut(progress);
            default -> sineIn(progress);
        };
    ...
    private double sineIn(double t) {
        return 1 - Math.cos(t * Math.PI / 2);
    }

    private double sineOut(double t) {
        return Math.sin(t * Math.PI / 2);
    }

    private double sineInOut(double t) {
