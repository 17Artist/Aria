...
public class Q3 implements IAriaObject {
    ...
    @AriaObjectConstructor("Q3")
    public Q3(InvocationData data) {
        if (data.size() >= 4) {
    ...
    public double get(InvocationData data) {
        ...
        double value = switch (type) {
            case 1 -> cubicOut(progress);
            case 2 -> cubicInOut(progress);
            default -> cubicIn(progress);
        };
    ...
    private double cubicIn(double t) {
        return t * t * t;
    }
    ...
    private double cubicInOut(double t) {
        if (t < 0.5) {
            return 4 * t * t * t;
        } else {
            t = 2 * t - 2;
