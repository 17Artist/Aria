...
val.inRange2 = x ~~ 1..10          // 同上：`1..10` 是 `Range(1, 10)` 的字面量简写
val.eq = x ~~ 'hello'              // 非 Range 类型，等价于 x == 'hello'
```

`a..b` 是**区间字面量**，等价于 `Range(a, b)`（左闭右开，步长 1）；需要自定义步长时仍用 `Range(a, b, step)`。可直接用于 `~~` 和 `for-in`。

> Range 为**左闭右开**区间：`Range(1, 10)`（或字面量 `1..10`）包含 1 到 9，不含 10。与 `for (i in Range(...))` 的遍历范围一致。
