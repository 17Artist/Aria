...
输入表达式后回车即可执行，结果以 `= ` 前缀显示（结果为 `none` 时不打印）：
...
> 每次回车是一次独立执行，**裸名临时变量不跨行保留**；REPL 会话中要复用的变量请使用
> `var.`（如上例）——`var` 存储挂在 REPL 的 Context 上，跨行持久。
...
```
>>> [1, 2][5]
Error: 单元: [eval] 运行时错误, 位于第 1 行, 到第 1 行
请检查: [1, 2][5]
错误信息: 列表索引越界: 5 (size=2) (line 1:1)
>>>
```
...
Aria 支持将编译后的 IR 程序序列化为 `.ariac` 二进制文件，实现预编译和快速加载。

> 二进制由文件头 magic bytes（`AR\x00\x01`）识别，**按内容而非扩展名**判定；`.ariac` 是预编译产物的推荐后缀（源码用 `.aria`）。
...
// 写入二进制文件（不含源码映射）
AriaFileWriter.write(routine.getProgram(), Path.of("myScript.ariac"));

// 写入二进制文件（含源码映射，用于调试）
AriaFileWriter.write(routine.getProgram(), Path.of("myScript.ariac"), true);
```

> `AriaFileWriter` 不强制后缀（写到任意路径都行），`.ariac` 是约定；加载时按 magic 识别。
...
IRProgram program = AriaFileReader.read(Path.of("myScript.ariac"));
```
